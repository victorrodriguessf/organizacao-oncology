...
test('private pages and data require authentication', async ({page,request})=>{
 for(const route of ['/api/guide','/api/auth/me'])expect((await request.get(route)).status()).toBe(401);
 await page.goto('/profissionais');await expect(page).toHaveURL('/login');
...
test('login error, password visibility, deep-link restoration, reload and logout',async({page,context})=>{
 ...
 await page.goto('/profissionais');await page.getByLabel('Login',{exact:true}).fill('admin');await page.getByLabel('Senha',{exact:true}).fill('errada');
 ...
 await page.getByLabel('Senha',{exact:true}).fill('admin');await page.getByRole('button',{name:'Acessar base de dados'}).click();await expect(page).toHaveURL('/profissionais');await expect(page.getByRole('heading',{name:'Profissionais da rede',exact:true})).toBeVisible();
 ...
 await page.reload();await expect(page.getByRole('heading',{name:'Profissionais da rede',exact:true})).toBeVisible();
 expect((await request.get('/api/guide')).status()).toBe(200);
 await page.goto('/unidades/COMN');await expect(page.getByRole('heading',{name:'Clínica de Oncologia e Mastologia',exact:true})).toBeVisible();
 ...
 expect((await request.get('/api/guide',{headers:{Cookie:`oncology_session=${cookie.value}`}})).status()).toBe(401);
...
test('forged and expired JWTs are rejected; other users cannot log in',async({request})=>{
 ...
 expect((await request.get('/api/guide',{headers:{Cookie:`oncology_session=${forged}`}})).status()).toBe(401);
 ...
 expect((await request.get('/api/guide',{headers:{Cookie:`oncology_session=${expired}`}})).status()).toBe(401);
});
test('expired session clears the screen and logout reaches other tabs',async({page,context})=>{
 ...
 await page.clock.install();await page.goto('/');await expect(page.getByRole('heading',{name:'Pessoas que cuidam. Informações que conectam.'})).toBeVisible();
 ...
 const other=await context.newPage();await other.goto('/');await expect(other.getByRole('heading',{name:'Pessoas que cuidam. Informações que conectam.'})).toBeVisible();
...
test('login is responsive and honors reduced motion',async({page})=>{
 ...
 await page.getByLabel('Login',{exact:true}).fill('admin');await page.getByLabel('Senha',{exact:true}).fill('admin');await page.getByLabel('Senha',{exact:true}).press('Enter');await expect(page.getByRole('heading',{name:'Pessoas que cuidam. Informações que conectam.'})).toBeVisible();
});
